...
from train_MLP import parse_config, get_features
...
def makeTriggerTurnOn(l1_pt_test,l1_pt_predicted,reco_pt_test,thresholds,options):
    ...
    if options.extend_pt_range:
     hreco_den_zoom = rt.TH1F("hreco_den_zoom","hreco_den_zoom",200,100,500)
     hrecoTrue_num_zoom = rt.TH1F("hrecoTrue_num_zoom","hrecoTrue_num_zoom",200,100,500)
     hrecoPred_num_zoom = rt.TH1F("hrecoPred_num_zoom","hrecoPred_num_zoom",200,100,500)
    else:
    ...
    c_zoom.cd()
    if options.extend_pt_range: frame_zoom = c_zoom.DrawFrame(100,0,500,1.4)
    else: frame_zoom = c_zoom.DrawFrame(0,0,300,1.4)
...
def makeRates(options,yamlConfig,model,njets=-1):
    ...
    all_features = ["pT", "eta", "phi", "Had", "Em", "HoE", "iEt","PUEt","jetPUDonutEt0","jetPUDonutEt1","jetPUDonutEt2","jetPUDonutEt3", "RawEt", "SeedEt"]
    print("All inputs:")
    print(all_features)      
    ...
    print("Found",l1_pt_predict[l1_pt_predict>1023].shape[0],"jets at saturation after DNN correction")
    l1_pt_predict[l1_pt_predict>1023] = 1023.5 #Emulate saturation

    plt.figure()
    plt.semilogy()
    plt.hist(l1_pt, bins=100, color='purple', histtype='step', label='Standard correction')
    plt.hist(l1_pt_predict, bins=100, color='green', histtype='step', label='DNN correction')
    plt.xlabel('L1 E_{T} [GeV]')
    plt.legend(loc='upper right')
    plt.savefig(options.outputDir+'/L1pt-makeRates.png')
    ...
    nEvents = l1_pt.shape[0]
    print("Loop over",nEvents,"events...")
    ...
    if options.rateValue != 0:
     ...
     for ix in range(1,singleJetRates_emu.GetNbinsX()+1):
   	   if singleJetRates_emu.GetXaxis().GetBinLowEdge(ix) == options.threshold:
   		   rate = singleJetRates_emu.GetBinContent(ix)
   		   break      

     for ix in range(1,singleJetRates_regression.GetNbinsX()+1):
   	   if singleJetRates_regression.GetXaxis().GetBinLowEdge(ix) == options.threshold:
   		    rate_regression = singleJetRates_regression.GetBinContent(ix)
   		    break

     print("Rate at fixed single jet threshold",options.threshold,"before regression",rate)
     print("Rate at fixed single jet threshold",options.threshold,"after regression",rate_regression)
    ...
    if options.rateValue != 0:
     ...
     hline_fixed_rate = rt.TLine(0,options.rateValue,cut,options.rateValue)
     vline_fixed_rate = rt.TLine(cut,0,cut,options.rateValue)
     hline_fixed_rate_regression = rt.TLine(0,options.rateValue,cut_regression,options.rateValue)
     vline_fixed_rate_regression = rt.TLine(cut_regression,0,cut_regression,options.rateValue)
 
    else:
    
     hline_fixed_rate = rt.TLine(0,rate,options.threshold,rate)
     vline_fixed_rate = rt.TLine(options.threshold,0,options.threshold,rate)
     hline_fixed_rate_regression = rt.TLine(0,rate_regression,options.threshold,rate_regression)
     vline_fixed_rate_regression = rt.TLine(options.threshold,0,options.threshold,rate_regression)     
     
    hline_fixed_rate.SetLineStyle(2)
    hline_fixed_rate.SetLineColor(rt.kBlack)
    hline_fixed_rate.SetLineWidth(2)
    hline_fixed_rate.Draw()
    vline_fixed_rate.SetLineStyle(2)
    vline_fixed_rate.SetLineColor(rt.kBlack)
    vline_fixed_rate.SetLineWidth(2)
    vline_fixed_rate.Draw()
        
    hline_fixed_rate_regression.SetLineStyle(2)
    hline_fixed_rate_regression.SetLineColor(rt.kBlue)
    hline_fixed_rate_regression.SetLineWidth(2)    
    hline_fixed_rate_regression.Draw()
    vline_fixed_rate_regression.SetLineStyle(2)
    vline_fixed_rate_regression.SetLineColor(rt.kBlue)
    vline_fixed_rate_regression.SetLineWidth(2)    
    vline_fixed_rate_regression.Draw()
...
def makeResolution(reco_pt_test,reco_eta_test,l1_ptcorr,l1_pt_predict,options,which="pt"):
    ...
    if options.extend_pt_range:   
     ptbins = np.arange(20,150,5,dtype=float).tolist()
     ptbins.extend(np.arange(150,300,10,dtype=float).tolist())
     ptbins.extend(np.arange(300,500,20,dtype=float).tolist())
     ptbins.extend(np.arange(500,700,50,dtype=float).tolist())
     ptbins.extend(np.arange(700,1000,100,dtype=float).tolist())     
    binsy = []
    minx=-2.
    maxx=1.
    ...
    if which == 'eta': draw(c_final_ratio,l,[mg_ratio],"AP",'ratio_resolution_vs_%s_1D'%which,xtitle='Offline Jet #eta',ytitle='#sigma / (1-#mu)',xmin=0.,xmax=0.,ymin=0.,ymax=0.5)
    else: draw(c_final_ratio,l,[mg_ratio],"AP",'ratio_resolution_vs_%s_1D'%which,xtitle='Offline Jet p_{T}',ytitle='#sigma / (1-#mu)',xmin=0.,xmax=0.,ymin=0.,ymax=0.5)
...
def makeHTResolution(dr,n_matched_jets,reco_pt_test,reco_eta_test,l1_ptcorr,l1_eta,l1_pt_predict,options):
    ...
    for e,n in enumerate(n_matched_jets):
     ...
     if HTreco > 30. and HTL1 > 30.:
      ...
      hres.Fill(HTreco,(HTreco-HTL1)/HTreco)
      hresp.Fill(HTreco,HTL1/HTreco)
     if HTreco > 30. and HTL1predict > 30.:
      ...
      hres_regression.Fill(HTreco,(HTreco-HTL1predict)/HTreco)
      hresp_regression.Fill(HTreco,HTL1predict/HTreco)
    ...
    c.cd()
    c.SetLeftMargin(0.15)
    draw(c,0,[hres],"COLZ",'HT_resolution_2D',xtitle='Offline H_{T}',ytitle='( H_{T}^{reco}-H_{T}^{L1} ) / H_{T}^{reco}')
    
    c_regression = getCanvas('c_2D_regression', '', 0)
    c_regression.cd()
    c_regression.SetLeftMargin(0.15)
    draw(c_regression,0,[hres_regression],"COLZ",'HT_resolution_regression_2D',xtitle='Offline H_{T}',ytitle='( H_{T}^{reco}-H_{T}^{L1} ) / H_{T}^{reco}')

    #2d response
    c_resp = getCanvas('c_2D_resp', '', 0)
    c_resp.cd()
    c_resp.SetLeftMargin(0.15)
    draw(c_resp,0,[hresp],"COLZ",'HT_response_2D',xtitle='Offline H_{T}',ytitle='H_{T}^{L1} / H_{T}^{reco}')

    c_resp_regression = getCanvas('c_2D_resp_regression', '', 0)
    c_resp_regression.cd()
    c_resp_regression.SetLeftMargin(0.15)
    draw(c_resp_regression,0,[hresp_regression],"COLZ",'HT_response_regression_2D',xtitle='Offline H_{T}',ytitle='H_{T}^{L1} / H_{T}^{reco}')

    #now study projections        
    x = array('d',[])
    xErr = array('d',[])
    mean = array('d',[])
    meanErr = array('d',[])
    ...
    for b in range(1,hres.GetNbinsX()+1):
     ...
     text = pt.AddText("#mu = %.2f, #sigma = %.2f"%(mean_regression[-1],width_regression[-1]))
     text.SetTextColor(rt.kBlue)
     draw(c1,l,[proj,proj_regression],"HIST","debug_fitHTres_bin%i"%(b),xtitle='( H_{T}^{reco}-H_{T}^{L1} ) / H_{T}^{reco}',ytitle='',xmin=0.,xmax=0.,ymin=0.,ymax=0.,pt=pt)
    ...
    c_final_mean = getCanvas('c_final_mean', '', 0)
    c_final_mean.SetLeftMargin(0.15)
    draw(c_final_mean,l,[mg_mean],"AP",'mean_HTresolution_1D',xtitle='Offline H_{T}',ytitle='#mu [ ( H_{T}^{reco}-H_{T}^{L1} ) / H_{T}^{reco} ]',xmin=0.,xmax=0.,ymin=-0.5,ymax=0.5)
    ...
    c_final_sigma = getCanvas('c_final_sigma', '', 0)
    c_final_sigma.SetLeftMargin(0.15)
    draw(c_final_sigma,l,[mg_sigma],"AP",'sigma_HTresolution_1D',xtitle='Offline H_{T}',ytitle='#sigma [ ( H_{T}^{reco}-H_{T}^{L1} ) / H_{T}^{reco} ]',xmin=0.,xmax=0.,ymin=0.,ymax=1.0)
    ...
    c_final_ratio = getCanvas('c_final_ratio', '', 0)
    c_final_ratio.SetLeftMargin(0.15)
    draw(c_final_ratio,l,[mg_ratio],"AP",'ratio_HTresolution_1D',xtitle='Offline H_{T}',ytitle='#sigma / (1-#mu)',xmin=0.,xmax=0.,ymin=0.,ymax=0.5)
    ...
    c_final_resp_mean = getCanvas('c_final_resp_mean', '', 0)
    c_final_resp_mean.SetLeftMargin(0.15)
    draw(c_final_resp_mean,l,[mg_resp_mean],"AP",'mean_HTresponse_1D',xtitle='Offline H_{T}',ytitle='#mu [ H_{T}^{L1} / H_{T}^{reco} ]',xmin=0.,xmax=0.,ymin=0.,ymax=2.)
    ...
    c_final_resp_sigma = getCanvas('c_final_resp_sigma', '', 0)
    c_final_resp_sigma.SetLeftMargin(0.15)
    draw(c_final_resp_sigma,l,[mg_resp_sigma],"AP",'sigma_HTresponse_1D',xtitle='Offline Jet p_{T}',ytitle='#sigma [ H_{T}^{L1} / H_{T}^{reco} ]',xmin=0.,xmax=0.,ymin=0.,ymax=1.)
...
def makeHTRates(options,yamlConfig,model,njets):
    ...
    l1_pt_predict = getPredictions(features_val,yamlConfig,model)
    l1_pt_predict[l1_pt_predict>1023] = 1023.5
    ...
    nEvents = l1_pt_predict.shape[0] #l1jetTree.shape[0]
    print("DEBUG nJets",nEvents,nL1Jets.shape,l1_pt_predict.shape)
    HT = 0.
    HTpredict = 0.
    thisEvent = 0
    for e in range(nEvents):
    ...
    if options.rateHTValue!=0:
     ...
     for ix in range(1,HTRates_emu.GetNbinsX()+1):
   	   if HTRates_emu.GetXaxis().GetBinLowEdge(ix) == options.HTthreshold:
   		   rate = HTRates_emu.GetBinContent(ix)
   		   break      

     for ix in range(1,HTRates_regression.GetNbinsX()+1):
   	   if HTRates_regression.GetXaxis().GetBinLowEdge(ix) == options.HTthreshold:
   		    rate_regression = HTRates_regression.GetBinContent(ix)
   		    break

     print("Rate at fixed HT threshold",options.HTthreshold,"before regression",rate)
     print("Rate at fixed HT threshold",options.HTthreshold,"after regression",rate_regression)
    ...
    if options.rateHTValue != 0:
     ...
     hline_fixed_rate = rt.TLine(HTLo,options.rateHTValue,cut,options.rateHTValue)
     vline_fixed_rate = rt.TLine(cut,0,cut,options.rateHTValue)
     hline_fixed_rate_regression = rt.TLine(HTLo,options.rateHTValue,cut_regression,options.rateHTValue)
     vline_fixed_rate_regression = rt.TLine(cut_regression,0,cut_regression,options.rateHTValue)
 
    else:
    
     hline_fixed_rate = rt.TLine(HTLo,rate,options.HTthreshold,rate)
     vline_fixed_rate = rt.TLine(options.HTthreshold,0,options.HTthreshold,rate)
     hline_fixed_rate_regression = rt.TLine(HTLo,rate_regression,options.HTthreshold,rate_regression)
     vline_fixed_rate_regression = rt.TLine(options.HTthreshold,0,options.HTthreshold,rate_regression)     
...
def makeHTTriggerTurnOn(dr,dr_reco,n_matched_jets,l1_pt_test,l1_pt_predicted,reco_pt_test,l1_eta,reco_eta_test,thresholds,options):

    if options.extend_pt_range:
     hreco_den = rt.TH1F("hreco_den","hreco_den",20,0,1000)
     hrecoTrue_num = rt.TH1F("hrecoTrue_num","hrecoTrue_num",20,0,1000)
     hrecoPred_num = rt.TH1F("hrecoPred_num","hrecoPred_num",20,0,1000)
     hreco_den_zoom = rt.TH1F("hreco_den_zoom","hreco_den_zoom",50,0,1000)
     hrecoTrue_num_zoom = rt.TH1F("hrecoTrue_num_zoom","hrecoTrue_num_zoom",50,0,1000)
     hrecoPred_num_zoom = rt.TH1F("hrecoPred_num_zoom","hrecoPred_num_zoom",50,0,1000)
    else:
     ...
     hreco_den_zoom = rt.TH1F("hreco_den_zoom","hreco_den_zoom",60,0,300)
     hrecoTrue_num_zoom = rt.TH1F("hrecoTrue_num_zoom","hrecoTrue_num_zoom",60,0,300)
     hrecoPred_num_zoom = rt.TH1F("hrecoPred_num_zoom","hrecoPred_num_zoom",60,0,300)
    ...
    c = getCanvas('c', '', 0)
    c.cd()
    if options.extend_pt_range: frame = c.DrawFrame(0,0,1000,1.4)
    else: frame = c.DrawFrame(0,0,600,1.4)
    frame.GetYaxis().SetTitleOffset(0.9)
    frame.GetYaxis().SetTitle('Efficiency')
    ...
    c_zoom = getCanvas('c_zoom', '', 0)
    c_zoom.cd()
    if options.extend_pt_range: frame_zoom = c_zoom.DrawFrame(0,0,1000,1.4)
    else: frame_zoom = c_zoom.DrawFrame(0,0,300,1.4)
    frame_zoom.GetYaxis().SetTitleOffset(0.9)
    frame_zoom.GetYaxis().SetTitle('Efficiency')
...
if __name__ == "__main__":
    ...
    parser.add_option("--ext",'--extend_pt_range',action="store_true",dest="extend_pt_range", help="Extend pT range to 1 TeV in plots",default=False)
    ...
    if not options.split:
     X_test, y_test, reco_variables_test, l1_ptcorr_test, weights_test, n_matched_jets, dr, dr_reco = get_features(options, yamlConfig)
    else:
    ...
    l1_pt_predict[l1_pt_predict>1023] = 1023.5 #Emulate saturation
    print("Found",l1_pt_predict[l1_pt_predict>1023].shape,"jets at saturation after DNN correction")
    ...
    if options.threshold != 0:
     cut,cut_regression = options.threshold,options.threshold
     makeRates(options,yamlConfig,model,2000000)
    else: cut,cut_regression = makeRates(options,yamlConfig,model,2000000)
    ...
    HTcut,HTcut_regression = 0,0     
    if options.HTthreshold != 0:
     HTcut,HTcut_regression = options.HTthreshold,options.HTthreshold
     makeHTRates(options,yamlConfig,model,2000000)
    else: HTcut,HTcut_regression = makeHTRates(options,yamlConfig,model,2000000)

    if HTcut != 0 and HTcut_regression != 0: makeHTTriggerTurnOn(dr,dr_reco,n_matched_jets,l1_ptcorr,l1_pt_predict,reco_pt_test,l1_eta,reco_eta_test,[HTcut,HTcut_regression],options)
    makeHTResolution(dr,n_matched_jets,reco_pt_test,reco_eta_test,l1_ptcorr,l1_eta,l1_pt_predict,options)
